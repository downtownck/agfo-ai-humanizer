...
AGFO AI Humanizer, Microsoft Word içinde çalışan Office.js tabanlı bir metin insanlaştırma eklentisidir. Bu repo WordPress eklentisi değildir. v4.3.4 revizyonunda AGFO Novel Humanizer WordPress örneğindeki preset/prompt deneyimi Word add-in mimarisine uyarlanmıştır; add-in içindeki bağımsız AI Manager yapısına dokunulmamıştır.
...
## v4.3.4 Prompt Katmanı Düzeltmesi
...
## v4.3.4 Cache-Busting Düzeltmesi

- `manifest.xml` sürümü 4.3.4 yapıldı.
- `SourceLocation` adreslerine `?v=4.3.4` parametresi eklendi.
- `taskpane.html` içindeki `taskpane.js` çağrısı `taskpane.js?v=4.3.4` olarak güncellendi.
...
## 4.3.4
- Yazar rehberleri artık tam dosya olarak değil, sadeleştirilmiş kısa stil özü olarak prompta eklenir.
- Örnek sahne, aforizma ve uzun alıntı blokları prompttan ayıklanır.
- Üslup Kaynağı için “Üslup yok” seçeneği eklendi; işlem modu, hazır üslup ve yazar rehberi net ayrıldı.
